...
import { runEngine, handleEngineFailure, type EngineResult } from "./engine";
import { autoTransitionAfterExec, autoTransitionAfterReview, inferPhaseFromFiles } from "./phases";
import { gitPush, commitState } from "./git";
  ...
  switch (phase) {
    ...
    case "exec": {
      ...
      if (existsSync(progressPath)) {
        const section = extractCurrentSection(
...
function updateStateIteration(
  ...
  return updateState(taskDir, (s) => {
    ...
    if (usage) {
      newState.usage = {
        total_cost_usd:
          s.usage.total_cost_usd + (usage.cost_usd ?? 0),
        total_duration_ms:
          s.usage.total_duration_ms + (usage.duration_ms ?? 0),
        total_turns: s.usage.total_turns + (usage.num_turns ?? 0),
        total_input_tokens:
          s.usage.total_input_tokens + (usage.input_tokens ?? 0),
        total_output_tokens:
          s.usage.total_output_tokens + (usage.output_tokens ?? 0),
        total_cache_read_input_tokens:
          s.usage.total_cache_read_input_tokens +
          (usage.cache_read_input_tokens ?? 0),
        total_cache_creation_input_tokens:
          s.usage.total_cache_creation_input_tokens +
  ...
  while (true) {
    ...
    if (!shouldContinue(state, iteration, opts)) {
      if (state.phase === "done") {
        const progressPath = join(taskDir, "PROGRESS.md");
        if (existsSync(progressPath)) {
          const { checked, unchecked } = countProgress(
            readFileSync(progressPath, "utf-8"),
          );
          console.log(
    ...
    if (shouldContinue(state, iteration, opts) && opts.delay > 0) {
      console.log(
        `  [wait] Sleeping ${opts.delay}s before next iteration...\n`,
      );
