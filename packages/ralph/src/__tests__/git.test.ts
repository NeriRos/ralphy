...
const { getCurrentBranch, gitAdd, gitCommit, gitPush, commitState } =
  await import("../git");

let tempDir: string;

beforeEach(() => {
  tempDir = mkdtempSync(join(tmpdir(), "ralph-git-test-"));
...
describe("commitState", () => {
  test("adds and commits state.json", () => {
    ...
    expect(mockExecSync).toHaveBeenCalledWith(
      'git add "/tasks/test/state.json"',
      { stdio: "pipe" },
    );
    expect(mockExecSync).toHaveBeenCalledWith(
      'git commit -m "phase transition"',
      { stdio: "pipe" },
    );
  });
