  ...
  switch (current) {
    case "research": {
      if (!existsSync(join(taskDir, "RESEARCH.md"))) {
        throw new Error(
          "Cannot advance from research — RESEARCH.md does not exist yet",
        );
      }
    ...
    case "plan": {
      if (
        ...
        throw new Error(
          "Cannot advance from plan — PLAN.md and PROGRESS.md must both exist",
        );
      }
      const progress = readFileSync(join(taskDir, "PROGRESS.md"), "utf-8");
      const { unchecked } = countProgress(progress);
      if (unchecked === 0) {
        throw new Error(
          "Cannot advance to exec — PROGRESS.md has no unchecked items",
        );
      }
 ...
 */
export function autoTransitionAfterExec(
  state: State,
  taskDir: string,
): State {
 ...
 */
export function autoTransitionAfterReview(
  state: State,
  taskDir: string,
): State {
  if (!existsSync(join(taskDir, "PROGRESS.md"))) return state;
