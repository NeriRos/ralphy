...
import { countProgress } from "ralph-core/progress";
...
export function showBanner(state: State, opts: ShowBannerOpts): void {
  ...
  console.log(` ${chalk.bold("Branch:")}     ${state.metadata.branch ?? "main"}`);
  ...
  if (opts.iterationDelay && opts.iterationDelay > 0) {
    console.log(` ${chalk.bold("Delay:")}      ${opts.iterationDelay}s between runs`);
  }
